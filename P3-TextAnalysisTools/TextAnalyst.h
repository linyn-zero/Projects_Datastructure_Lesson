...
*/
class TextAnalyst {
	map<string, int> wordsByDict;                // 词库。记录所有出现过的单词，key是单词，value是出现次数
	map<string, map<int,int>> wordsLine;         // 单词与其出现的行序号与次数
...
public:
	...
	void ShowAllWord_byVECTOR(const vector<string>&) const;         // 功能实现接口：展示所有单词及其出现次数（从给定的数组中，按数组的顺序）
	void InputSearch() const;                                // 功能2：输入想要查询的内容
	void showKeyword() const;                                      // 功能3：展示保留字及其出现次数（按次数排序）
	void displayTheWord(const string&) const;                // 功能实现接口：展示一个特定单词的出现行数与每行个数.参数确保存在
